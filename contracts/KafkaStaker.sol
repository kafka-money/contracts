...
contract KafkaStaker is ReentrancyGuard, Ownable {
    ...
    struct Balances {
        uint256 total;   // total = locked + unlocked + earned?
        uint256 unlocked;
        uint256 locked;
        uint256 earned;
    }
    ...
    function rewardPerToken(address _rewardsToken) external view returns (uint256) {
        uint256 supply = _rewardsToken == address(stakingToken) ? lockedSupply.add(earningSupply) : totalSupply;
    ...
    function earned(address account) external view returns (RewardData[] memory _rewards) {
        ...
        for (uint256 i = 0; i < _rewards.length; i++) {
            ...
            uint256 balance = i == 0 ? bal.locked.add(bal.earned) : balances[account].total;
            uint256 supply = i == 0 ? lockedSupply.add(earningSupply) : totalSupply;
    ...
    function earnedBalances(
        ...
        for (uint i = 0; i < earnings.length; i++) {
            if (earnings[i].unlockTime > block.timestamp) {
                if (idx == 0) {
                    lockData = new LockedBalance[](earnings.length - i);
                }
                lockData[idx] = earnings[i];
                idx++;
            } else {
                unlocked = unlocked.add(earnings[i].amount);
            }
        }
        return (unlocked, lockData);
    }
    ...
    function withdrawableBalance(
        ...
        if (bal.earned > 0) {
            ...
            penaltyAmount = bal.earned.sub(amountWithoutPenalty).mul(2).div(3);
        }
    ...
    function stakeWithPermit(uint256 amount, bool lock, uint deadline, uint8 v, bytes32 r, bytes32 s) external {
        // permit
    ...
    function stake(uint256 amount, bool lock) public nonReentrant updateReward(msg.sender) {
    ...
    function mint(address user, uint256 amount) external updateReward(user) {
        ...
        totalSupply = totalSupply.add(amount);
        earningSupply = earningSupply.add(amount);
        Balances storage bal = balances[user];
        bal.total = bal.total.add(amount);
    ...
    function doPenalty(uint256 penaltyAmount) private {
        if (penaltyAmount > 0) {
            uint256 burnAmount = penaltyAmount.div(4); // 1/6 == (2/3)/4
            ERC20Burnable(address(stakingToken)).burn(burnAmount); // 1/6 to burn
            _notifyReward(address(stakingToken), penaltyAmount.sub(burnAmount)); // 1/2 to notify
        }
    }
    ///@dev need to dec TotalSupply and earningSupply outside
    function _withdrawEarned(uint256 amount) private {
        require(amount > 0, "Cannot withdraw 0");
        Balances storage bal = balances[msg.sender];
        uint256 earnedBefore = bal.earned;
        uint256 penaltyAmount;
        {
            ...
            for (uint i = 0; ; i++) {
                ...
                if (penaltyAmount == 0 && userEarnings[msg.sender][i].unlockTime > block.timestamp) {
                    uint256 needLocked = remaining.mul(3); // 3/3
                    penaltyAmount = needLocked.sub(remaining); // 2/3 penalty
                    require(bal.earned >= penaltyAmount, "Insufficient balance after penalty");
                    bal.earned = bal.earned.sub(penaltyAmount);
                    if (bal.earned == 0) {
                        delete userEarnings[msg.sender];
                        break;
                    }
                    remaining = needLocked; // 3/3
                }
        ...
        uint256 deltaSupply = earnedBefore.sub(bal.earned);
        bal.total = bal.total.sub(deltaSupply);
        earningSupply = earnedBefore.sub(deltaSupply);
        totalSupply = totalSupply.sub(deltaSupply);
        stakingToken.safeTransfer(msg.sender, amount);
        doPenalty(penaltyAmount);
        emit Withdrawn(msg.sender, amount);
    }


    function withdrawEarned(uint256 amount) external nonReentrant updateReward(msg.sender) {
        _withdrawEarned(amount);
    }

    function _withdrawDemand(uint256 amount) private {
        require(amount > 0, "Cannot withdraw 0");
        Balances storage bal = balances[msg.sender];
        require(amount <= bal.unlocked, "Insufficient demand balance");
    ...
    function withdrawDemand(uint256 amount) external nonReentrant updateReward(msg.sender) {
        _withdrawDemand(amount);
    }

    // Withdraw staked tokens
    // First withdraws unlocked tokens, then earned tokens. Withdrawing earned tokens
    // incurs a 2/3 penalty which is distributed based on locked balances.
    function _withdraw(uint256 amount) private {
        require(amount > 0, "Cannot withdraw 0");
        Balances storage bal = balances[msg.sender];
        uint256 unlockedPart = Math.min(bal.unlocked, amount);
        if(unlockedPart > 0) {
            _withdrawDemand(unlockedPart);
        }
        uint256 earnedPart = amount.sub(unlockedPart);
        if(earnedPart > 0) {
            _withdrawEarned(earnedPart);
        }
    }
    ...
    function notifyRewardAmount(address _rewardsToken, uint256 reward) external updateReward(address(0)) {
        require(rewardDistributors[_rewardsToken][msg.sender], "onlyDistributor");
    ...
    modifier updateReward(address account) {
        ...
        uint256 balance;
        uint256 supply = lockedSupply.add(earningSupply);
        ...
        if (account != address(0)) {
            ...
            Balances storage bal = balances[account];
            rewards[account][token] = _earned(account, token, bal.locked.add(bal.earned), supply);
