...
contract LPStaker is ReentrancyGuard, Ownable {
    ...
    function earlyPenalty(uint256 amount)
        public
        pure
        returns (uint256 eraly, uint256 penaltyBurned, uint256 penaltyNotified)
    {
        eraly = amount / 3;
        uint256 penalty = amount - eraly;
        penaltyBurned = penalty/4;
        penaltyNotified = penalty - penaltyBurned;
    }
    ...
    function getRewardWithLimit(bool force, uint256 size) public updateReward(msg.sender) {
        ...
        if (force) {
            (uint256 early, uint256 burned, uint256 notified) = earlyPenalty(totalLocked);
